...
const Navbar = ({ toggledrawer, seteditcreatechanelbtn }) => {
  ...
  //Start Task 1: User Points Code
  const userPoints = useSelector(state => state.userPointsReducer.currentuser?.points);
  // const videosWatched = useSelector(state => state.userPointsReducer.user?.videosWatched);
  //End Task 1: User Points Code 
  ...
  >
    ...
    />
...

        {
          currentuser? (
            <>
              <div className="user_points_rewards" > {/* onClick={userPoints()} */}
    <svg
    className="w-6 h-6 text-gray-800 dark:text-white"
    aria-hidden="true"
    xmlns="http://www.w3.org/2000/svg"
  ...
  <p className="user_points">{userPoints}</p>
        </div>
            </>
          ) : (<></>)
        }
